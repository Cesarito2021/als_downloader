...
uploads are limited to 1 GB. These are application controls, not universal Shiny limits.
...
The Explore preview accepts one selected LAS/LAZ result with a known HTTP file size up to 1 GB.
...
It downloads sequentially, permits four tiles/600 MB per campaign (300 MB each) and clips both clouds to the intersection of their provider footprints and the AOI, at most 1 km². Empty or oversized overlap is rejected before transfer.
