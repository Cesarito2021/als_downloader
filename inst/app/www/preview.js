(function () {
  ...
  function viewer(c) {
    ...
    function draw() {
      ...
      const colors=palettes[palette],lut=Array.from({length:256},(_,n)=>{
      ...
      colors.forEach((p,i)=>gradient.addColorStop(i/(colors.length-1),'rgb('+p.join(',')+')'));
      ctx.fillStyle='rgba(8,14,20,.9)';ctx.fillRect(10,h-70,245,65);
      ctx.fillStyle=gradient;ctx.fillRect(20,h-40,180,7);
      ctx.fillStyle='#c7d6df';ctx.fillText(palette+' · source elevation · Z ×'+exag,20,h-50);
      ctx.fillText(Number(origin[2]).toFixed(1),20,h-15);
      ctx.textAlign='right';ctx.fillText((Number(origin[2])+extent[2]).toFixed(1),200,h-15);
    }
    ...
    return {load(data){
      ...
      fit();},
      update(data){if(data.exaggeration!=null)exag=data.exaggeration;if(palettes[data.palette])palette=data.palette;if(data.fit)fit();else draw();}};
  }
