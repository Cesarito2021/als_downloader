...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function spin(ts){if(ts-lastSpin>60){const elapsed=Math.min(ts-lastSpin,150);lastSpin=ts;if(autorotate&&!document.hidden&&canvas.offsetParent){lon+=elapsed*.00625;draw();}}requestAnimationFrame(spin);}
  function draw(){
