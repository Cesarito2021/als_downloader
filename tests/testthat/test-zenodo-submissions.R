...
test_that("the public form can queue a proposal but cannot invoke reviewer actions", {
  ...
  shiny::testServer(function(input,output,session)zenodo_submission_server(input,output,session,queue),{
    ...
    expect_equal(zenodo_submissions(queue)$status,"pending")
    session$setInputs(zenodo_tracking_id=zenodo_submissions(queue)$id,zenodo_track=1)
    expect_match(output$zenodo_tracking_status,"Awaiting ALS Downloader team review")
    expect_match(output$zenodo_tracking_status,"10.5281/zenodo.12345",fixed=TRUE)
    expect_false(grepl("contact|private@",output$zenodo_tracking_status))
