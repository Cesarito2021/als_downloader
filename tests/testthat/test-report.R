...
test_that("unknown sizes do not produce a zero-duration promise and figures embed", {
  ...
  html <- paste(readLines(path, warn = FALSE), collapse = "\n")
  expect_true(grepl("Size not reported by the provider", html, fixed=TRUE))
...
test_that("report dates and storage preserve precision without false zeros", {
  expect_equal(report_date_label("2022-08-03", "2022-08-03"), "2022-08-03")
  expect_equal(report_date_label("2022-01-01", "2022-12-31"), "2022")
  expect_equal(report_date_label("2022-08-03", "2022-08-05"), "2022-08-03 to 2022-08-05")
  expect_equal(report_date_label(NA, NA), "Not reported")
  expect_equal(report_storage_label(100e6), "100.00 MB")
  expect_equal(report_storage_label(c(1e9, 500e6)), "1.50 GB")
  expect_equal(report_storage_label(c(NA, -1)), "Size not reported by the provider")
  expect_equal(report_storage_label(c(100e6, NA)), "100.00 MB reported; 1 file(s) with size not reported")
