...
selection_script <- function(tiles) {
  ...
  c(paste0("# ", als_welcome_message()),
    "# Optional: run this script in local R/RStudio to download your app selection.",
    "# You do not need this script when using Download ALS data in the app.",
    "",
    "# Load the installed package (see the README for installation).",
    'library(ALSdownloadeR)',
    "",
    "# Set the destination folder; change this path if needed.",
    'output_dir <- file.path(path.expand("~"), "ALS-Downloads")',
    "",
    "# workers: requested number of simultaneous downloads, not dedicated CPU cores.",
    "# The actual count also depends on available cores and provider_limit.",
    "workers <- 2L",
    "# provider_limit: maximum simultaneous transfers allowed for this run.",
    "# Keep this at 2 unless the provider permits more connections.",
    "provider_limit <- 2L",
    "",
    "# Data sources: the files selected in the app, with their original URLs and credits.",
    "# No new search is needed. Original files are downloaded without clipping.",
    "# Review the recorded provider licences and access conditions.",
    paste0("tiles <- ", paste(utils::capture.output(dput(tiles)), collapse = "\n")),
    "",
    "# Download in parallel to your computer; the app can be closed.",
    'downloads <- download_als_data(',
    '  tiles, output_dir = output_dir, workers = workers,',
    '  provider_limit = provider_limit, mode = "local"',
    ')',
    "",
    "# Inspect per-file results. Rerun with the same folder to reuse verified files.",
    "print(downloads)")
}
