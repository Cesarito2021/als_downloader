    ...
    shiny::div(class = "als-layout",
      shiny::conditionalPanel("input.enter_map > 0", class = "als-sidebar-toggle",
        ...
        shiny::div(class = "als-section",
          ...
          shiny::actionButton("download", "Download ALS data", class = "als-action-btn"),
          ...
          shiny::tabPanel("Explore",
            ...
            shiny::conditionalPanel("output.report_ready === 'yes'",
              shiny::div(class="als-campaign-selection",
                ...
                shiny::helpText("Replaces the current selection across all result pages. Then use Download ALS data. Acquisition year is the final year; filename references are labelled in the table. Missing values appear as NA."))),
            ...
            shiny::tags$details(class="als-optional-script",
              shiny::tags$summary("Optional: download with R"),
              shiny::helpText("Prefer R/RStudio? Export your selected files as a script and run it on your computer. Download ALS data already downloads the files directly; no script is required."),
              shiny::downloadButton("export_script", "Download R script")),
  ...
  server <- function(input, output, session) {
      ...
      tryCatch({
        outcome <- shiny::withProgress(message = "Searching sources", value = .2, {
          out <- lapply(providers, function(p) tryCatch(
            find_als_data(state$aoi, if (p == "zenodo-approved") "contributed" else p,
        ...
        if (nrow(result)) {
          ...
          year_label <- acquisition_year_label(result)
          levels_all <- sort(unique(year_label))
    ...
    shiny::observeEvent(state$tiles, {
      ...
      choices <- c("All years"="all", stats::setNames(known, known),
        if ("unknown" %in% years) c("NA (year unavailable)"="unknown"))
    ...
    output$tiles <- DT::renderDT({
      ...
      table <- als_metadata_columns(sf::st_drop_geometry(state$tiles))
      table$campaign_id <- campaign_display(table)
      table$acquisition_year <- acquisition_year_label(table)
      DT::datatable(table[c("filename", "campaign_id", "acquisition_year", "provider", "date_status", "source_url", "metadata_url", "license_url", "citation")],
        colnames=c("ALS file", "Campaign / dataset", "Acquisition year", "Source", "Date evidence", "Source record", "Metadata", "License", "Producer / citation"),
        rownames=FALSE, selection=list(mode="multiple",selected=NULL), filter="top", class="stripe hover compact",
        options=list(scrollX=TRUE,pageLength=8,columnDefs=list(list(targets=5:8,visible=FALSE))))
    })
    ...
    output$report_selection_summary<-shiny::renderText({
      ...
      paste(nrow(x), "files | Total download size:", report_storage_label(x$size_bytes), "Select files to narrow the report.")
    })
    ...
    show_tile_details <- function(licenses=FALSE) {
      ...
      details <- unique(sf::st_drop_geometry(als_metadata_columns(x))[c("dataset", "provider", "citation", "license_url", "date_status", "date_scope", "source_url", "metadata_url")])
      shiny::showModal(shiny::modalDialog(title=if(licenses) "License and attribution" else "Product information",
        ...
        lapply(seq_len(nrow(details)), function(i) shiny::tags$section(
          ...
          if (!licenses) shiny::p(paste("Acquisition evidence:", details$date_status[i], "| Scope:", details$date_scope[i])),
          if (!licenses) lapply(c("source_url", "metadata_url"), function(field) {
            link <- details[[field]][i]
            if (!is.na(link) && grepl("^https://", link))
              shiny::tags$a(href=link, target="_blank", rel="noopener noreferrer",
                if(field=="source_url") "Source record " else "Acquisition metadata ")
          }),
    ...
    output$selection_summary <- shiny::renderText({
      ...
      rows <- if(nrow(x)) x else state$tiles
      rows <- rows[!duplicated(asset_identity_url(rows$url)),,drop=FALSE]
      paste0(sprintf("Selected %s of %s tiles. ", nrow(x), nrow(state$tiles)),
        if(!nrow(x)) "All results: " else "", "Total download size: ", report_storage_label(rows$size_bytes))
    })
    ...
    output$export_script <- shiny::downloadHandler(filename = "download-als-data.R", content = function(file) {
      ...
      tryCatch({
        ...
        if (mode == "hosted" && anyNA(rows$size_bytes)) {
          for (i in which(is.na(rows$size_bytes))) {
            url <- rows$url[i]
            url <- asset_access_url(rows[i,,drop=FALSE])
