...
asset_identity_url <- function(urls) {
  vapply(urls,function(url) {
    u<-httr::parse_url(url)
    if(length(u$query)) {
      auth<-grepl("^(sig|se|sp|sv|sr|st|skoid|sktid|skt|ske|sks|skv|token|access_token|x-amz-.*)$",names(u$query),ignore.case=TRUE)
      u$query<-u$query[!auth]
      if(length(u$query)) u$query<-u$query[order(names(u$query))]
    }
    u$fragment<-NULL; httr::build_url(u)
...
redact_urls_in_text <- function(x) gsub("(https?://[^[:space:]\"'<>?#]+)[?#][^[:space:]\"'<>]*", "\\1", x)
...
asset_path <- function(tile, output_dir) {
  key <- digest::digest(paste(tile$provider, tile$tile_id, asset_identity_url(tile$url)), algo = "sha256", serialize = FALSE)
