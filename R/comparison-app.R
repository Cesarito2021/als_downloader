...
comparison_server <- function(input, output, session, state, mode, hosted_lock) {
  ...
  availability <- shiny::reactive({side<-if(is.null(input$compare_side_m))100 else input$compare_side_m
    if(is_local()) {x<-local_tile();if(inherits(x,"error"))return(list(ready=FALSE,message=conditionMessage(x)))
      ...
      if(length(a)!=1L || length(b)!=1L || is.na(a) || is.na(b) || !any((p$a==a & p$b==b) | (p$a==b & p$b==a)))
  ...
  shiny::observeEvent(list(state$tiles, state$aoi, is_local()), {
    rows<-if(is_local()) {if(is.null(state$tiles))integer() else seq_len(nrow(state$tiles))} else unique(c(pairs()$a,pairs()$b))
  ...
  shiny::observeEvent(list(input$epoch_a,pairs(),is_local()), {
    if(is_local())return()
    p<-pairs();chosen<-suppressWarnings(as.integer(input$epoch_a));rows<-unique(c(p$b[p$a %in% chosen],p$a[p$b %in% chosen]))
  ...
  choose_download <- function(key) {
    ...
    shiny::showNotification("Only this source's tiles are selected. Review them and use Download ALS data.")
  }
