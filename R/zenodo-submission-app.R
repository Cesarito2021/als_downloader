zenodo_submission_ui <- function() shiny::tagList(
  ...
  shiny::selectInput("zenodo_boundary_source","Coverage polygons",c("Upload a coverage file"="upload")),
  ...
  shiny::conditionalPanel("input.zenodo_has_boundary == 'no'",
    ...
    shiny::numericInput("zenodo_distance","Centre to each side (metres)",value=500,min=1,max=50000),
    shiny::helpText("500 m gives a 1 x 1 km square. Maximum: 50,000 m to each side."),
