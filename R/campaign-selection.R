...
# The displayed acquisition year is the final acquisition year. Older saved
# tables without this field retain their original interval-based selection.
tile_year_membership <- function(tiles) {
  if (is.null(tiles) || !nrow(tiles)) return(list())
  lapply(seq_len(nrow(tiles)), function(i) {
    if ("acquisition_year" %in% names(tiles)) {
      year <- tiles$acquisition_year[i]
      return(if (is.na(year)) "unknown" else as.character(year))
    }
