...
* Add a "Statement of need and related software" section to the README, positioning the package against point-cloud processing tools (lidR) and individual provider portals. Replace two stray RStudio project files left over from early working names (lidar_app.Rproj, ot_pc_app.Rproj) with a single alsdownloader.Rproj.
* Add a README banner (docs/images/banner.svg) using the app's own colour language (light purple / pale yellow point clouds on black), the author's name near the top, functional R-CMD-check/lifecycle/license badges, and rename "Install and launch" to "Getting started".
